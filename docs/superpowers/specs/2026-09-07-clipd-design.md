...
| 2 | Gaming PC OS? | **Windows** — confirmed on the tailnet as `desktop-amtr56i` / `100.66.55.18` |
...
Deployment follows the conventions in `plan.md` §4: `/home/batman/clipd/` with its
...
Content-Type: multipart/form-data
  ...
  meta = {"kind":"clip","capture_uuid":"<uuid4>","game":"Counter-Strike 2",
          "game_exe":"cs2.exe","source_host":"desktop-amtr56i","captured_at":1757260800}
```
...
above, and the storage layout. Deployed to `/home/batman/clipd/` via compose.
...
SSH key auth from the MacBook to `batman@100.72.75.62` is not currently working
(`Permission denied (publickey)`). Required before anything deploys to midget.

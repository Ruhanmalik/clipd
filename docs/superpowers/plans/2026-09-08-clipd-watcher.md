...
- The daemon runs on **`gaming-pc` / `<gaming-pc-ip>`** (Windows). Development and unit testing happen on macOS, so **no module outside `clipwatch/platform/windows.py` may import `win32*` or `pywin32`.**
- Server endpoint: `POST http://clipd-server.tailnet-name.ts.net:8000/ingest`, bearer auth.
...
TOML = """
server_url = "http://clipd-server.tailnet-name.ts.net:8000"
...
def test_load_reads_toml(tmp_path):
    ...
    assert cfg.server_url == "http://clipd-server.tailnet-name.ts.net:8000"
    assert cfg.watch_dir == Path("C:/captures")
    assert cfg.source_host == "gaming-pc"
...
watch_dir = "C:/Users/<you>/Videos/clipd"
...
def cfg(tmp_path):
    ...
    toml.write_text(f'server_url = "http://clipd-server:8000"\nwatch_dir = "{tmp_path.as_posix()}"\n'
                    'source_host = "gaming-pc"\n')
...
def test_successful_upload_returns_the_url(cfg, job):
    def handler(request):
        return httpx.Response(200, json={"id": "aB3xY9z", "url": "http://clipd-server:8000/v/aB3xY9z"})

    result = upload(cfg, job, client=transport(handler))
    assert result == UploadResult(ok=True, url="http://clipd-server:8000/v/aB3xY9z",
...
def test_drain_deletes_the_local_file_only_after_a_200(cfg, tmp_path, buffer):
    ...
    def ok(config, j, client=None):
        return UploadResult(True, "http://clipd-server:8000/v/abc", "abc", False)
...
**Interfaces:**
- Consumes: everything above
- Produces: a running watcher on `gaming-pc`

Requires SSH to the gaming PC (see Prerequisite) and a reachable clipd. Until Step 1 is deployed to clipd-server, point `server_url` at a clipd running locally.
...
```bash
ssh <user>@<gaming-pc-ip> 'mkdir -p C:/clipwatch'
rsync -av --exclude .venv --exclude __pycache__ client/ <user>@<gaming-pc-ip>:C:/clipwatch/
```

- [ ] **Step 6: Install and run there**

```bash
ssh <user>@<gaming-pc-ip> 'cd C:/clipwatch && py -3.12 -m venv .venv && .venv/Scripts/pip install -e ".[dev]"'
ssh <user>@<gaming-pc-ip> 'cd C:/clipwatch && .venv/Scripts/python -m pytest -q'
```
...
```bash
ssh <user>@<gaming-pc-ip> 'cd C:/clipwatch && .venv/Scripts/python -c "
from clipwatch.platform import get_adapter
a = get_adapter()
print(type(a).__name__, a.foreground())
print(\"clipboard:\", a.set_clipboard(\"http://clipd-server:8000/v/test\"))

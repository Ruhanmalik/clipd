...
**Architecture:** A single FastAPI service behind one SQLite index, deployed by Docker Compose to `/home/<user>/clipd/` on clipd-server. Uploads stream to disk under `clips/<game-slug>/YYYY/MM/<id>.mp4`; the only video work the server ever does is extracting one thumbnail frame with ffmpeg. A background timer prunes oldest-first against a size budget. Pure-function modules (`slug`, `ids`), an I/O layer (`db`, `storage`, `media`, `notify`), and a thin route layer (`app`) keep each file to one responsibility.
...
- clipd binds port **8000** — confirmed free on clipd-server (`plan.md` §3).
- Deploy target is **`/home/<user>/clipd/`**, one directory per service (`plan.md` §4).
...
SSH key auth from the MacBook to `<user>@<server-ip>` must work before Task 10. Currently refused (`Permission denied (publickey)`). Fix with `ssh-copy-id <user>@<server-ip>`. Tasks 1–9 need no server access.
...
# Binary multipliers: "50GB" means 50 GiB. The disk budget on clipd-server is what
...
`write_stream` must never load the whole upload into memory — a 90-second clip can be several hundred megabytes and clipd-server has 14 GiB total.
...
async def test_notify_capture_posts_to_topic_with_click_action(monkeypatch):
    ...
    ok = await notify_capture(
        ...
        click_url="http://clipd-server:8000/v/aB3xY9z",
    )
    ...
    assert sent["headers"]["Click"] == "http://clipd-server:8000/v/aB3xY9z"
...
### Task 10: Containerize and deploy to clipd-server
...
- Produces: a running service at `http://clipd-server.tailnet-name.ts.net:8000`

Requires SSH key auth to `<user>@<server-ip>` (see Prerequisite).
...
services:
  clipd:
    ...
    ports:
      # 8000 is free on clipd-server; 3000/3001/5055/6767/7878/8080/8096/8989/9696
...
- [ ] **Step 4: Verify the build locally before touching clipd-server**
...
curl -sS -H "Authorization: Bearer $TOKEN" \
     -F file=@/tmp/test.mp4 \
     -F 'meta={"kind":"clip","capture_uuid":"local-test-1","game":"Counter-Strike 2","game_exe":"cs2.exe","source_host":"laptop"}' \
...
ID=$(curl -sS -H "Authorization: Bearer $TOKEN" -F file=@/tmp/test.mp4 \
  -F 'meta={"kind":"clip","capture_uuid":"local-test-2","game":"Counter-Strike 2","source_host":"laptop"}' \
...
- [ ] **Step 7: Deploy to clipd-server**

```bash
ssh <user>@<server-ip> 'mkdir -p /home/<user>/clipd'
cd <repo-root>
rsync -av --exclude data --exclude .env --exclude .git --exclude __pycache__ \
      ./ <user>@<server-ip>:/home/<user>/clipd/

ssh <user>@<server-ip> 'cd /home/<user>/clipd && cp .env.example .env && \
  sed -i "s|^INGEST_TOKEN=.*|INGEST_TOKEN=$(openssl rand -base64 32)|" .env && \
...
```bash
curl -fsS http://clipd-server.tailnet-name.ts.net:8000/healthz
ssh <user>@<server-ip> 'cd /home/<user>/clipd && docker compose ps && docker compose logs --tail 30'
```
...
phone, run the Step 5 ingest against `clipd-server.tailnet-name.ts.net`, and confirm the

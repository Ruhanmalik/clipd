...
> Handoff spec. Written on the homelab box (`clipd-server`) so a later Claude session on
...
### `clipd-server` — the homelab (deploy target for the server)
...
- On the tailnet as `laptop` / `<laptop-ip>`, macOS.
- Code here, deploy to `clipd-server` over SSH (port 22 is open on the tailnet).

---

## 3. Network

Tailscale tailnet, MagicDNS suffix `tailnet-name.ts.net`:

| Host | Tailscale IP | MagicDNS |
|---|---|---|
| clipd-server (server) | `<server-ip>` | `clipd-server.tailnet-name.ts.net` |
...
```
[ Gaming PC — 7900X / RTX 4070 ]              [ clipd-server — over Tailscale ]
...
Single service, `/home/<user>/clipd/`. Language is open — Go (single static binary,
...
5. POST to `http://clipd-server.tailnet-name.ts.net:8000/ingest` with the bearer token.
...
**Step 1 — `clipd` on clipd-server.** Ingest + auth + SQLite + thumbnail + ntfy.
